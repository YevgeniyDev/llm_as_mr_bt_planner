...
%%  - default "pure" mode gives the model the declarative scenario, schema, and task-agnostic method;
...
This paper studies whether a large language model (LLM) can act as the \emph{planner} for a heterogeneous multi-robot team, generating synchronized, executable Behavior Trees (BTs) from a natural-language instruction and a declarative scene. The LLM infers the task graph, capability-aware assignment, synchronization constraints, and one BT per robot, while deterministic code only validates and simulates. Invalid or deadlocked plans are corrected by the LLM itself. Three task-agnostic reliability levers are evaluated: a general back-chaining method in the prompt, best-of-$N$ sampling, and two-stage generation. The \emph{pure} setting supplies the instruction, explicit symbolic scenario and capability model, output schema, and general method; an \emph{assisted} ablation additionally injects producer hints. Generated trees are exported to BehaviorTree.CPP XML and can be replayed in MuJoCo, while goal success remains symbolically evaluated.
...
Two deterministic computations perform planning-style causal reasoning and are therefore \emph{off by default}: (i) precomputed dependency hints and (ii) candidate-producer names in validator feedback. Pure mode still receives the natural-language instruction, explicit initial and goal states, objects, locations, robot capability libraries with preconditions and effects, the output schema, and a task-agnostic planning method. Assisted mode adds producer-specific scaffolding as an upper-bound ablation.
...
We evaluate whether the framework generates valid, synchronized, goal-reaching BTs for heterogeneous teams. The primary instrument is symbolic simulation. Each LLM condition uses a fixed model snapshot, temperature, correction budget, best-of-$N$ value, and published seed schedule over 30 trials. Generated trees are exported to BehaviorTree.CPP XML and replayed in MuJoCo; MuJoCo is embodiment evidence rather than an independent goal-success oracle.
...
\label{sec:baselines}
Beyond the internal ablation, we compare against three methods. The two LLM baselines use the same model, scenarios, validator, and blocking-guard simulator as the proposed method. MRBTP is reported under a native protocol because its FAILURE-returning Condition semantics are incompatible with our RUNNING-returning synchronization guards; native rows are explicitly labelled rather than described as same-simulator measurements.
\begin{itemize}
    ...
    \item \textbf{MRBTP} \cite{cai2025mrbtp}: the authors' released symbolic planner, run on ported versions of our scenarios. Native success requires no timeout, an expanded condition grounded in the initial state, and extractable per-robot trees; ``not timed out'' alone is not success.
\end{itemize}

\subsection{Metrics}
We report validity and goal-success rates with Wilson 95\% intervals; synchronization-, capability-, causal-, and structural-error rates; deadlock and timeout rates; mean correction rounds and wall time; plan size; Action and Condition counts; synchronization edges; executed actions; and symbolic makespan. MRBTP has zero LLM correction rounds and uses native planning time and outcomes.

\subsection{Results}
Tables~\ref{tab:results} and~\ref{tab:diagnostics} are generated directly from an immutable result snapshot containing the matrix, commit SHA, raw per-trial outputs, aggregates, environment metadata, and SHA-256 checksums. This prevents manual transcription of values into the manuscript.
...
First, planning success is symbolic: MuJoCo replays the same trees but does not yet feed contact, grasp, or collision outcomes back as the goal-success oracle. Second, the framework does not yet control real robots; BehaviorTree.CPP XML export and the ROS backend remain integration seams. A conservative incident policy specifies safe stop, reacquisition, quarantine-and-replacement, reassignment, and operator escalation for dropped or damaged items, but perception-backed detection and physical recovery skills remain future work. Third, LLM generation is stochastic, so reliability is reported over repeated trials. Finally, correctness is relative to the declarative model and does not transfer beyond its fidelity.
